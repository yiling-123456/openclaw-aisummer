        ...
        if not seg["citations"]:
            ...
            for cit in seg["citations"]:
                ...
                if review is None:
                    all_valid = False
                    break
                if cit["keyword"] not in review["content"]:
                    all_valid = False
                    break
            if all_valid:
                output_parts.append(seg["text"])
            # 校验失败 → 整段跳过（不输出声明，不输出引用标记）
...
def postprocess_citations(
    ...
    3. 逐条校验：序号存在？关键词在原文中？
    4. **默认模式**（``show_all=False``）：只保留**全部通过**校验的段落，
       且去掉 ``@…@`` 标记，输出干净文本。未通过的段落整段丢弃。
    5. **舒展模式**（``show_all=True``，对应 CLI ``-a`` 参数）：保留完整
       输出，对失败引用原地标注 ``⚠️``，末尾附加引用验证报告。
